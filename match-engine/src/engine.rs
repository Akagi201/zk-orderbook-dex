...
impl MatchingEngine {
    ...
    fn get_ask_side(&mut self, symbol: &str) -> &mut OrderBookSide {
        &mut self.markets.entry(symbol.to_string()).or_default().ask_side
    }

    fn get_bid_side(&mut self, symbol: &str) -> &mut OrderBookSide {
        &mut self.markets.entry(symbol.to_string()).or_default().bid_side
    }
